...
public class StatisticServlet extends HttpServlet {
    ...
    System.out.println("GET action: " + action); // ✅ log
...
private void handleImportStatistic(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
        System.out.println("handleImportStatistic chạy rồi ✅");

        String fromRaw = request.getParameter("from");
        String toRaw = request.getParameter("to");

        System.out.println("From: " + fromRaw + ", To: " + toRaw); // ✅ log
        ...
        // Thống kê chính
        List<ExportStatistic> mainList = dao.getImportStatistic(from, to);

        // Thống kê biểu đồ phụ
        List<ExportStatistic> importByCategory = dao.getImportByCategory(from, to);
        List<ExportStatistic> importByDate = dao.getImportByDate(from, to);
    ...
    throws ServletException, IOException {
        // Lấy action để xử lý phân nhánh chức năng
...
String action = request.getParameter("action");
    ...
    private void handleExportStatistic(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<ExportStatistic> list = dao.getExportStatistic(from, to, categoryId, userId);

            // Gửi danh sách thống kê sang JSP
    ...
    // Hàm tiện ích để tránh lỗi NumberFormatException
