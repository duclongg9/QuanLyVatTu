...
public class deleteSupplier extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
       ...
       String idParam = request.getParameter("sid");

        if (idParam != null) {
            try {
                int id = Integer.parseInt(idParam);
                SupplierDAO dao = new SupplierDAO();
                boolean success = dao.softDeleteSupplier(id);

                if (success) {
                    // Chuyển hướng về danh sách nhà cung cấp (cập nhật mới)
                    response.sendRedirect("suppliercontroller");
                } else {
                    // Trả về trang lỗi nếu không xóa được
